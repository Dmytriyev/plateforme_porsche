...
const deleteUser = async (req, res) => {
  try {
    ...
    return res.status(200).json({ message: "Utilisateur et toutes ses données ont été supprimés" });
  } catch (error) {
    console.log(error);
    res.status(500).json({ message: "Erreur serveur", error: error });
  }
  ...
  try {
    ...
    if (panier) {
      ...
      const total = ligneCommandes.reduce((sum, line) => {
        const prix = line.type_produit_v ? line.type_produit_v.prix : line.type_produit_a.prix;
    ...
    const profile = {
      ...
      stats: {
        ...
        totalPorsches: await Model_porsche_actuel.countDocuments({ user: userId }),
        totalCommandes: await Commande.countDocuments({ user: userId, status: false })
      }
    };
  ...
  try {
    ...
    if (!reservation) {
      return res.status(404).json({ 
        message: "Réservation introuvable ou n'appartient pas à cet utilisateur" 
      });
    }

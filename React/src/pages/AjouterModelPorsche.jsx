/**
 * pages/AjouterModelPorsche.jsx — Admin: création d'un modèle Porsche.
 *
...
const AjouterModelPorsche = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const voitureSelectionnee = voitures.find(
        (v) => v._id === formData.voiture,
      );
      if (!voitureSelectionnee) {
      ...
      const nouvelleVoiture = await voitureService.createVoiture({
        ...
        description:
          formData.description || `${voitureSelectionnee.nom_model} d'occasion`,
        photo_voiture: [],
      });
      ...
      const nouveauModel =
        await modelPorscheService.createModelPorsche(dataToSend);
      ...
      if (photos.length > 0 && nouveauModel._id) {
        ...
        try {
          await modelPorscheService.ajouterPhotos(
            nouveauModel._id,
            photoFormData,
          );
        } catch (photoError) {
          // Journaliser l'erreur pour ne pas masquer un échec d'upload
          warn("Erreur lors de l'ajout des photos du modèle :", photoError);
        }
      }

  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const errorMessage =
          err.message || "Erreur lors du chargement des variantes";
          ...
          {variantes.length === 0 ? (
            ...
            <div className="catalogue-modeles-grid-occasion">
              {variantesFiltrees.map((variante) => {
                ...
                if (isNeuf) {
                  if (
                    variante.photo_porsche &&
                    Array.isArray(variante.photo_porsche) &&
                    variante.photo_porsche.length > 0
                  ) {
                    const validPhotos = variante.photo_porsche.filter(
                      (p) => p && (p.name || p._id),
                    );
                  ...
                  if (!photoPrincipale && variante.photo_voiture) {
                    if (
                      ...
                      variante.photo_voiture.length > 0
                    ) {
                      const validPhotos = variante.photo_voiture.filter(
                        (p) => p && (p.name || p._id),
                      );
                      if (validPhotos.length > 2) {
                        photoPrincipale = validPhotos[2];
                      ...
                      typeof variante.photo_voiture === "object" &&
                      variante.photo_voiture.name
                    ) {
                  ...
                  if (!photoPrincipale && variante.voiture?.photo_voiture) {
                    if (
                      Array.isArray(variante.voiture.photo_voiture) &&
                      variante.voiture.photo_voiture.length > 0
                    ) {
                      const validPhotos = variante.voiture.photo_voiture.filter(
                        (p) => p && (p.name || p._id),
                      );
                      if (validPhotos.length > 2) {
                        photoPrincipale = validPhotos[2];
                      ...
                      typeof variante.voiture.photo_voiture === "object" &&
                      variante.voiture.photo_voiture.name
                    ) {
                  ...
                  if (
                    !photoPrincipale &&
                    variante.voiture_base?.photo_voiture
                  ) {
                    if (
                      Array.isArray(variante.voiture_base.photo_voiture) &&
                      variante.voiture_base.photo_voiture.length > 0
                    ) {
                      const validPhotos =
                        variante.voiture_base.photo_voiture.filter(
                          (p) => p && (p.name || p._id),
                        );
                      if (validPhotos.length > 2) {
                        photoPrincipale = validPhotos[2];
                      ...
                      typeof variante.voiture_base.photo_voiture === "object" &&
                      variante.voiture_base.photo_voiture.name
                    ) {
                ...
                const _specifications = variante.specifications || {};
                      ...
                      {(variante.prix_base || variante.prix_calcule) > 0 ? (
                        <>
                          ...
                          <span className="catalogue-prix-montant">
                            {formatPrice(
                              variante.prix_base || variante.prix_calcule,
                            )}
                          </span>
                        </>
                      ) : (
